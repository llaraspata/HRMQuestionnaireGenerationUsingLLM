...
class ModelResultVisualizer:
    ...
    # Constructor
    # ------------
    def __init__(self, project_root, task="Survey", prompt_version="1.0", model="GPT"):
        result_dir = os.path.join(project_root, "results", task, prompt_version, model)
    ...
    def get_best_and_worst_experiments(self):
        bw_conversion_errors_df = ModelResultVisualizer._get_best_and_worst(self.errors, "CONVERSION_ERROR_RATE")
        bw_json_errors_df = ModelResultVisualizer._get_best_and_worst(self.errors, "JSON_ERROR_RATE")
        bw_intraquestionnaire_similarity_df = ModelResultVisualizer._get_best_and_worst(self.intraquestionnaire_similarity, "INTRAQSTN_ROUGE_L_F1_SCORE")
        bw_semantic_metrics_df = ModelResultVisualizer._get_best_and_worst(self.semantic_metrics, "FINAL_SCORE")
        bw_serendipity_scores_df = ModelResultVisualizer._get_best_and_worst(self.serendipity_scores, "AVG_SERENDIPITY_SCORE")
        bw_qst_type_variability_df = ModelResultVisualizer._get_best_and_worst(self.qst_type_variability, "AVG_QST_TYPE_VARIABILITY")
    ...
    def plot_general_correlation_matrix(self):
        ModelResultVisualizer._plot_correlation_matric(self.all_results.loc[:, self.all_results.columns != 'EXPERIMENT_ID'])
    ...
    def plot_correlation_matrix_by_config(self, title, model="", has_full_params=-1, shots=-1, temperature=-1, frequency_penalty=-1):
        ...
        ModelResultVisualizer._plot_correlation_matric(df.loc[:, df.columns != 'EXPERIMENT_ID'], title)
